...
### 무료 vs threddi.com (오픈코어)

이 저장소는 **AGPL-3.0 오픈소스**이며, 위 17개 도구는 **전부 무료·완전 로컬**로 동작한다 (데이터는 외부로 전송되지 않음).

아래 고급 분석은 호스팅 백엔드가 필요해 **[threddi.com](https://threddi.com) 유료 플랜 전용**이다. 로컬에서 호출하면 안내 메시지를 반환한다:

| 유료 전용 기능 | 설명 |
|----------------|------|
| 관계 그래프 (relationships) | 상호작용 네트워크·핵심 관계 분석 |
| 전환 퍼널 (conversion funnel) | 노출 → 참여 → 전환 단계 분석 |
| 코치 인사이트 (coach insights) | 성과 기반 맞춤 코칭 |
| 콘텐츠 믹스 (content mix) | 주제·포맷 배분 최적화 |
| 글쓰기 추천 (writing recommendation) | 다음 글 주제·각도 제안 |
| 일간 콘텐츠 플랜 (daily content plan) | 날짜별 발행 계획 |
...
### Free vs threddi.com (open core)

This repository is **AGPL-3.0 open source**, and the 17 tools above are **fully free and entirely local** (no data leaves your machine).

The advanced analytics below require a hosted backend and are **[threddi.com](https://threddi.com) paid-plan only**. Calling them locally returns an informational message:

| Paid-only feature | Description |
|-------------------|-------------|
| Relationships | Interaction network and key-relationship analysis |
| Conversion funnel | Impression → engagement → conversion stages |
| Coach insights | Performance-based personalized coaching |
| Content mix | Topic/format allocation optimization |
| Writing recommendation | Next-post topic and angle suggestions |
| Daily content plan | Date-by-date publishing plan |
